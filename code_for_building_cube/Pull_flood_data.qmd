...
```{r pull_flood_points}
flood_csv <- glue("/vsicurl/https://floodobservatory.colorado.edu/temp/FloodArchive.xlsx") |>
 vect() |>
  as.data.frame() |>
...
ggplot(data=flood_csv, aes(color=Severity)) +
  geom_sf(cex=0.1)
```

```{r plot_flood_point_MainCause}
ggplot(data=flood_csv, aes(color=MainCause))  +geom_sf(cex=0.1)
```

```{r plot_flood_point_displaced}
ggplot(data=flood_csv, aes(color=Displaced))  +geom_sf(cex=0.1)
```

```{r plot_flood_point_Dead}
ggplot(data=flood_csv, aes(color=Dead))  +geom_sf(cex=0.1)
```


```{r plot_flood_point_Validation}
ggplot(data=flood_csv, aes(color=Validation))  +geom_sf(cex=0.1)
```


```{r plot_flood_point_Ended}
ggplot(data=flood_csv, aes(color=Ended))  +geom_sf(cex=0.1)
```


```{r plot_flood_point_Area}
ggplot(data=flood_csv, aes(color=Area))  +
  geom_sf(cex=0.1) 
```
